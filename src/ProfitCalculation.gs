 ...
 * 設定値の更新（利益計算シート用）
 */
function updateProfitCalculationSettings() {
  try {
    ...
    // 為替レート手動設定フラグの更新
    updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_TOGGLE), settings.exchangeRateManual);
    
    // 手動為替レート値の更新
    updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_VALUE), settings.exchangeRateManualValue);
    
    console.log('利益計算シートの設定値を更新しました');
    
  } catch (error) {
    console.error('設定値更新中にエラーが発生しました:', error);
  }
}
...
/**
 * 為替レート管理の検証と更新
 * 仕様参照: doc/Joom利益計算機能仕様書.md 3.7
 */
function validateAndUpdateExchangeRate() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(SHEET_NAMES.PROFIT);
    if (!sheet) {
    ...
    // 現在の設定を取得
    const settings = loadSystemSettings();
    const isManual = settings.exchangeRateManual;
    const manualValue = settings.exchangeRateManualValue;
    
    console.log('=== 為替レート管理検証 ===');
    console.log('手動設定:', isManual);
    console.log('手動値:', manualValue);
    
    // シート上の値を確認
    const sheetManualFlag = sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_TOGGLE).getValue();
    const sheetManualValue = sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_VALUE).getValue();
    const sheetAutoValue = sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_AUTO_VALUE).getValue();
    const sheetFinalValue = sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_FINAL_VALUE).getValue();
    
    console.log('シート手動フラグ:', sheetManualFlag);
    console.log('シート手動値:', sheetManualValue);
    console.log('シート自動値:', sheetAutoValue);
    console.log('シート最終値:', sheetFinalValue);
    
    // 手動/自動切替の検証
    let needsUpdate = false;
    
    if (sheetManualFlag !== isManual) {
      console.log('手動フラグの不一致を検出、更新します');
      updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_TOGGLE), isManual);
      needsUpdate = true;
    }
    
    if (sheetManualValue !== manualValue) {
      console.log('手動値の不一致を検出、更新します');
      updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_VALUE), manualValue);
      needsUpdate = true;
    }
    
    // 自動値の検証（GoogleFinanceの値が有効かチェック）
    if (!isManual && (isNaN(sheetAutoValue) || sheetAutoValue <= 0)) {
      console.log('自動為替レートが無効です、デフォルト値にフォールバック');
      updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_AUTO_VALUE), 150.0);
      needsUpdate = true;
    }
    
    // 最終値の再計算確認
    const expectedFinalValue = isManual ? manualValue : sheetAutoValue;
    if (Math.abs(sheetFinalValue - expectedFinalValue) > 0.01) {
      console.log('最終値の不一致を検出、再計算します');
      // 数式を再設定
    ...
    if (needsUpdate) {
      console.log('為替レート設定を更新しました');
    } else {
      console.log('為替レート設定は正常です');
    }
    
    return true;
    
  } catch (error) {
    console.error('為替レート検証中にエラーが発生しました:', error);
    return false;
  }
}
 ...
 * 為替レート手動/自動切替の実行
 */
function toggleExchangeRateMode() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheet = ss.getSheetByName(SHEET_NAMES.PROFIT);
    if (!sheet) return;
    
    const currentMode = sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_TOGGLE).getValue();
    const newMode = !currentMode;
    
    // シート上のフラグを更新
    updateCellSilently(sheet.getRange(PROFIT_CELLS.EXCHANGE_RATE_MANUAL_TOGGLE), newMode);
    
    // 設定シートの値も更新
    if (typeof updateSetting === 'function') {
      updateSetting('利益計算 為替レート手動設定', newMode.toString());
    }
    
    console.log(`為替レートモードを${newMode ? '手動' : '自動'}に切り替えました`);
    
    // 検証を実行
    validateAndUpdateExchangeRate();
    
  } catch (error) {
    console.error('為替レート切替中にエラーが発生しました:', error);
  }
}
