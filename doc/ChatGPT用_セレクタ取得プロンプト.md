...
```
以下の商品ページから、ウェブスクレイピングに必要なセレクタ情報を取得してください。

**対象URL**: https://jp.mercari.com/item/m19494603044

**重要（ChatGPT Plus / web.run環境向け）**:
- このURLを取得する際、**生のHTMLソース**（タグ・id/class/data-testid 等の属性をすべて含む完全なHTML）を取得してください
- テキスト要約やリーダーモードではなく、**ページの完全なHTML構造**が必要です
- 取得したHTMLから、価格・在庫に関連する要素の属性を確認し、実在するセレクタのみを提案してください
...
**出力例（HTML属性が取得できず、テキストのみの場合）**:
取得できた情報（ドメイン、表示テキスト例など）のみを記入し、セレクタ項目は「不明」としてください。表の後に以下を付けてください。
```
※ この環境ではHTMLの属性が取得できず、セレクタの検証ができませんでした。
  このサイトのセレクタ設定は、IT担当者や管理者に依頼して取得・登録してください。
```
...
6. ✅ HTML属性を確認できない場合は推測せず「不明」とし、IT担当者等への依頼を案内すること
...
- **実際に存在するセレクタのみを列挙してください**
  ...
  - HTML属性が取得できない環境では、推測でセレクタを出さず「不明」とし、IT担当者等への依頼を案内してください
...
### HTML属性が取得できない場合（web.runでテキストのみ取得される場合）

**現象**: 本文テキスト（例：「価格：10,450円（税込）」）までは取得できるが、この環境の web.run では HTMLタグの属性（id / class / data-testid など）が落ちた形で抽出されてしまい、「実在するCSSセレクタ」を検証して特定することができない。

**想定原因**:

- ChatGPTのプランやモデルによって、web.run（ウェブ検索・取得）の挙動が異なる
- リーダーモード相当のテキスト抽出が行われ、HTMLの属性情報が落ちている

**対処法（プロンプトのみで対応）**:

1. **プロンプトで生のHTML取得を明示**
   - 本プロンプトには既に「生のHTMLソースを取得してください」と記載済みです
   - 初回で失敗した場合、続けて次のように送信してみてください：
   ```
   もう一度、上記URLの**ページソース（生のHTML、タグと属性をすべて含む）**を取得し直してください。
   テキスト要約ではなく、完全なHTML構造が必要です。
   ```

2. **IT担当者・管理者に依頼**
   - プロンプトだけでは解決しない場合、そのサイトのセレクタ取得をIT担当者や管理者に一度依頼し、取得した内容を仕入れ元マスターに登録してチームで共有してください
   - （依頼を受けた方）Chrome DevTools: 対象要素を右クリック→Inspect→要素を右クリック→Copy→Copy selector でセレクタを取得できます
