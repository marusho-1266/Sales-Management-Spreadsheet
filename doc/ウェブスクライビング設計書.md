...
3.  **WRITE**: Python (Selenium) が「Googleフォーム」に結果CSVをアップロード。
4.  **UPDATE**: GASの「フォーム送信時トリガー」が発火し、CSV内容をスプレッドシートに反映。
...
#### D. `uploader.py` (データ書き込み)
*   **機能**: 結果CSVをGoogleフォーム経由で送信する。
*   **ロジック**:
    1.  スクレイピング結果を `data/upload_data.csv` として保存。
       *   CSVの列名は日本語（`仕入れ元URL`, `仕入れ価格`, `在庫ステータス`, `最終更新日時`）を使用
       *   エンコーディング: UTF-8（BOM付き推奨）
    2.  `browser` で `GOOGLE_FORM_URL` を開く。
    3.  `input[type='file']` 要素を特定し、CSVパスを `send_keys` する。
    4.  送信ボタン（`role='button'` 等で特定）をクリック。
    5.  送信完了画面を確認して終了。
    6.  **重要**: CSVファイルは日本語列名を使用し、GAS側の処理と整合性を保つ。

### 5.3 GAS側スクリプト仕様 (`WebScrapingFormHandler.gs`)
*   **ファイル名**: `src/WebScrapingFormHandler.gs`
*   **トリガー**: 「フォーム送信時 (onFormSubmit)」
*   **処理フロー**:
    1.  フォーム添付ファイル（CSV）のIDを取得。
    2.  `DriveApp` でCSVファイルを取得し、`parseCsvWithMultilineSupport`関数（既存実装）でデータを展開。
...
Googleフォーム経由で送信するCSV形式。**日本語列名を使用**する。
...
5.  **Step 5**: `src/uploader.py` の実装（Googleフォーム自動投稿）。
6.  **Step 6**: `main.py` で全体のオーケストレーション実装。
7.  **Step 7**: GASコード (`WebScrapingFormHandler.gs`) の実装。
