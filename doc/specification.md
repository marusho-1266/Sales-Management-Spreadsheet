...
| 返金額(円) | 数値 | 返金された金額 | V |
| Joom手数料(円) | 数値 | Joom手数料 | W |
| サーチャージ(円) | 数値 | 配送サーチャージ | X |
| 繁忙期料金(円) | 数値 | 繁忙期料金 | Y |
| 利益 | 数値 | 自動計算値 | Z |
| 最終為替レート | 数値 | 為替レート記録 | AA |
| 最終更新日時 | 日時 | データ更新日時 | AB |
| 備考・メモ | 文字列 | 備考・メモ | AC |
| Joom連携ステータス | 文字列 | 未連携/連携済み | AD |
| 最終出力日時 | 日時 | Joom CSV最終出力日時 | AE |
...
| セル | 項目 | データ型 | 入力/計算 | 説明 |
|------|------|----------|-----------|------|
| B8 | 利益額(¥) | 数値 | 計算 | 販売価格 - 仕入原価 - 送料 - Joom手数料 - サーチャージ - 繁忙期料金 + 返金額 |
| B9 | 利益率(%) | 数値 | 計算 | 利益額 / 販売価格 |
| B10 | 還付込利益額(¥) | 数値 | 計算 | 利益額 + 返金額 |
| B11 | 還付込利益率(%) | 数値 | 計算 | 還付込利益額 / 販売価格 |
