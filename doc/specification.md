...
- **手動入力**: 商品選択、数量、販売価格、仕入れ価格、送料、返金額（任意）を入力。純利益はK列の計算式 `=H-(I+J+P+Q+R)` で自動計算
...
- **手数料計算**: Joom手数料の自動計算
- **税計算**: VAT（付加価値税）の計算
- **純利益**: 全コスト要素（仕入原価・送料・Joom手数料・サーチャージ・繁忙期料金）を考慮した利益算出
- **返金管理**: 返金額は売上管理シートで注文単位に管理（商品単位の利益計算には含めない）
...
**売上管理**
- **注文データ入力**: 入力ウィンドウによる注文データの入力（返金額は任意項目、純利益はK列の計算式で自動計算）
- **Joom注文取得**（サブメニュー）
...
**更新履歴**:
- v1.4 (2026-03): 手動売上入力フォームに返金額（任意）フィールドを追加。手動入力時の純利益(K列)を静的値から計算式=H-(I+J+P+Q+R)に変更し、Joom API連携と統一
